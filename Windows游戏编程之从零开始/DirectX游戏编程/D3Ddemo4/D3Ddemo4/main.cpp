//-----------------------------------【程序说明】----------------------------------------------
//  程序名称:：D3Ddemo4
//	2018年11月 Create by XY
//  描述：Direct3D顶点缓存、索引缓存缓存四部曲
//----------------------------------------------------------------------------------------------
...
#define WINDOW_TITLE L"【致我们永不熄灭的游戏开发梦想】Direct3D索引缓存的故事 示例程序"		
#define SAFE_RELEASE(p) {if(p) {(p)->Release(); (p)=nullptr;} }		//定义一个安全释放宏，便于后面COM接口指针的释放

//-----------------------------------【全局变量声明部分】---------------------------------------
//	描述：全局变量的声明
//------------------------------------------------------------------------------------------------
LPDIRECT3DDEVICE9 g_pd3dDevice = nullptr;		//Direct3D设备对象
ID3DXFont* g_pFont = nullptr;					//字体COM接口
float g_FPS = 0.0f;								//一个浮点型的变量，代表帧速率
wchar_t g_strFPS[50];							//包含帧速率的字符数组
LPDIRECT3DVERTEXBUFFER9 g_pVertexBuffer = nullptr;	//顶点缓冲区对象
LPDIRECT3DINDEXBUFFER9		g_pIndexBuffer  = NULL;    // 索引缓存对象


//-----------------------------------【全局函数声明部分】---------------------------------------
//	描述：全局函数的声明，防止“未声明的标识”系列错误
//------------------------------------------------------------------------------------------------
LRESULT CALLBACK WndProc(HWND hwnd,UINT message,WPARAM wParam, LPARAM lparam);		//窗口过程函数
HRESULT Direct3D_Init(HWND hwnd);		//在这个函数中进行Direct3D的初始化
HRESULT Objects_Init(HWND hwnd);		//在这个函数中进行要绘制物体的资源初始化
VOID Direct3D_Render(HWND hwnd);		//在这个函数进行Direct3D渲染代码的书写
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nShowCmd)
{
	...
	{
		//MessageBox(hwnd,L"Direct3D初始化完成",L"XY的消息窗口",0);		//使用messagebox函数，穿件一个消息窗口
	}
	else
	{
		MessageBox(hwnd,L"Direct3D初始化失败",L"XY的消息窗口",0);		//使用messagebox函数，穿件一个消息窗口
	}

	//PlaySound(L"Final Fantasy XIII.wav", NULL, SND_FILENAME | SND_ASYNC|SND_LOOP); //循环播放背景音乐 
	//	MessageBox(hwnd, L"DirectX，等着瞧吧，我们来降服你了~!", L"XY的消息窗口", 0); //使用MessageBox函数，显示一个消息窗口
...
HRESULT Direct3D_Init(HWND hwnd)
{
	...
	//	描述：创建Direct3D接口对象，以便用该Direct3D对象创建Direct3D设备对象
	//--------------------------------------------------------------------------------------------------
	LPDIRECT3D9 pD3D = nullptr;		//Direct3D接口对象的创建
	if(nullptr == (pD3D = Direct3DCreate9(D3D_SDK_VERSION)))		//初始化Direct3D接口对象，并进行DirectX版本协商
		return E_FAIL;

	//-----------------------------------【Direct3D初始化四部曲之二，取信息】--------------------------------------
	//	描述：获取硬件设备信息
	//--------------------------------------------------------------------------------------------------
	D3DCAPS9 caps;		//用于描述显卡的所有参数
	int vp = 0;
	if(FAILED(pD3D->GetDeviceCaps(D3DADAPTER_DEFAULT,D3DDEVTYPE_HAL,&caps)))
		return E_FAIL;
	if(caps.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT)
		vp = D3DCREATE_HARDWARE_VERTEXPROCESSING;		//支持硬件定点运算，我们就采用硬件顶点运算
	else
		vp = D3DCREATE_SOFTWARE_VERTEXPROCESSING;		//不支持，就用软件顶点运算

	//-----------------------------------【Direct3D初始化四部曲之三，填内容】--------------------------------------
	//	描述：填充D3DPRESENT_PARAMETERS结构体
	//--------------------------------------------------------------------------------------------------
	...
	SAFE_RELEASE(pD3D)		//LPDIRECT3D9 接口对象的使命完成，我们将其释放掉
...
HRESULT Objects_Init(HWND hwnd)
{
	...
	srand(timeGetTime());		//用系统时间初始化种子


	//-----------------------------------【顶点缓存、索引缓存绘图四步曲之二】---------------------------------------
	//	描述：创建顶点缓存和索引缓存
	//------------------------------------------------------------------------------------------------
	if( FAILED( g_pd3dDevice->CreateVertexBuffer( 17*sizeof(CUSTOMVERTEX),		//这里应该只用17个点就行了
...
void Direct3D_Render(HWND hwnd)
{
	...
	//--------------------------------------------------------------------------------------
	g_pd3dDevice->SetRenderState(D3DRS_SHADEMODE,D3DSHADE_GOURAUD);		//设置渲染状态，高洛得着色模式
	//-----------------------------------【顶点缓存、索引缓存绘图四步曲之四】---------------------------------------
	//	描述：绘制图形
	//------------------------------------------------------------------------------------------------
	...
	g_pFont->DrawText(nullptr,g_strFPS,charCount,&formatRect,DT_TOP|DT_RIGHT,D3DCOLOR_XRGB(255,39,136));
