...
public:
	...
	//四个Set系列函数，注意它们的参数都有默认值NULL得，调用时不谢参数也可以
	VOID SetTargetPosition(D3DXVECTOR3* pLookat = nullptr);		//设置摄像机的目标观察位置向量
	VOID SetCameraPosition(D3DXVECTOR3* pVector = nullptr);		//设置摄像机所在的位置向量
	VOID SetViewMatrix(D3DXMATRIX* pMatrix = nullptr);			//设置取景变换矩阵
	VOID SetProjMatrix(D3DXMATRIX* pMatrix = nullptr);			//设置投影变换矩阵
